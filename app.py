...
def handle_like(message_id):
    ...
    if liked_message in user.likes:
        like = Likes.query.filter_by(
            user_id=user.id, message_id=liked_message.id).first()
        db.session.delete(like)
        db.session.commit()
        return redirect("/")
    else:
        like = Likes(user_id=user.id, message_id=liked_message.id)
        db.session.add(like)
        db.session.commit()
        return redirect("/")
...
def homepage():
    ...
    if g.user:
        feed_ids = [message.id for message in g.user.following] + [g.user.id]
        messages = (Message
                    .query
                    .filter(Message.user_id.in_(feed_ids))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
